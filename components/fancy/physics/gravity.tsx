  ...
  (
    ...
    const registerElement = useCallback(
      (id: string, element: HTMLElement, props: MatterBodyProps) => {
        ...
        const collisionPadding = props.collisionPadding ?? 0
        ...
        if (props.bodyType === "circle") {
          const radius = Math.max(width, height) / 2 + collisionPadding
        ...
        if (body) {
          const bodyIndex = bodiesMap.current.size
          const sideVelocity = ((bodyIndex % 7) - 3) * 0.14
          const fallVelocity =
            gravity.y >= 0
          ...
          Body.setVelocity(body, { x: sideVelocity, y: fallVelocity })
    ...
    const startElementSync = useCallback(() => {
    ...
    const stopElementSync = useCallback(() => {
      if (frameId.current) {
    ...
    const reset = useCallback(() => {
      ...
      syncElements()
      handleResize()
    }, [canvasSize.height, canvasSize.width, handleResize, stopEngine, syncElements])
