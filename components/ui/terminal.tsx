...
export const AnimatedSpan = ({
  ...
  useEffect(() => {
    ...
    if (sequence.activeIndex === itemIndex) {
      const startTimeout = setTimeout(() => setHasStarted(true), 0)
    ...
    >
      <div className="border-border flex flex-col gap-y-2 border-b p-4">
        <div className="flex flex-row gap-x-2">
          <div className="h-2 w-2 rounded-full bg-muted-foreground/25"></div>
          <div className="h-2 w-2 rounded-full bg-muted-foreground/25"></div>
          <div className="h-2 w-2 rounded-full bg-muted-foreground/25"></div>
        </div>
      </div>
      <pre className="p-4">
        <code className="grid gap-y-1 overflow-hidden whitespace-pre-wrap">
          {wrappedChildren}
        </code>
      </pre>
    </div>
